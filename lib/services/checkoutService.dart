...
class CheckoutService {
  ...
  Future<void> newCreditCardDetails(
      String cardNumber, String expiryDate, String cardHolderName) async {
    String uid = await _userService.getUserId();
    QuerySnapshot creditCardData = await _creditCardReference
  ...
  Future<List> listCreditCardDetails() async {
    ...
    QuerySnapshot<Map<String, dynamic>> cardData =
        await _creditCardReference.where('userId', isEqualTo: uid).get();
    String cardNumber;
  ...
  Future<List> listPlacedOrder() async {
    ...
    String uid = await _userService.getUserId();
    QuerySnapshot<Map<String, dynamic>> orders = await _orderReference
    ...
    for (DocumentSnapshot<Map<String, dynamic>> order in orders.docs) {
      ...
      for (int i = 0; i < order.data()['items'].length; i++) {
        ...
        DocumentSnapshot<Map<String, dynamic>> docRef =
            await _productReference.doc(order.data()['items'][i]['id']).get();
