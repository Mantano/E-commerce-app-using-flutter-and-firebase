...
class AdminService {
  ...
  void createSampleData() {
    ...
    loadAsset('assets/csv/CATEGORY_MOCK_DATA.csv').then((dynamic value) async {
      ...
      loadAsset('assets/csv/PRODUCTS_MOCK_DATA.csv')
          .then((dynamic value) async {
        List<List<dynamic>> tempProductData =
            const CsvToListConverter().convert(value);
        ...
        for (int i = 1; i < tempProductData.length; i++) {
          ...
          QuerySnapshot categorySnapshot = await _categoryReference
              .where('name', isEqualTo: categoryName)
              .get();
          String categoryId = categorySnapshot.docs[0].id;
          subCategorySnapshot = await _subCategoryReference
